(function() {
  ...
  style.textContent =
    ...
    '.adv-btn{background:var(--cyan);color:#050510;border:0;border-radius:3px;padding:7px 12px;cursor:pointer;font:10px var(--font-mono);font-weight:bold}' +
  ...
  (function() {
    ...
    function setupEvents(win) {
      ...
      win.querySelector('#ai-file-save').addEventListener('click', function(){var p=win.querySelector('#ai-file-path').value;API.writeFile(p,win.querySelector('#ai-file-editor').value).then(function(r){win.querySelector('#ai-file-status').textContent=r.success?'Saved '+p:(r.error||'Save failed');}).catch(function(e){win.querySelector('#ai-file-status').textContent='Backend Offline — run ./start-bos.sh. '+e.message;});});
      win.querySelector('#ai-file-edit').addEventListener('click', function(){var out=win.querySelector('#ai-file-preview');out.textContent='Generating edit…';var prompt='Path: '+win.querySelector('#ai-file-path').value+'\nInstruction: '+win.querySelector('#ai-file-instruction').value+'\nCurrent file:\n'+win.querySelector('#ai-file-editor').value;API.chat(prompt,[],'file-edit').then(function(r){filePreview=r.reply||'';out.textContent=filePreview;}).catch(function(e){out.textContent='Backend Offline — run ./start-bos.sh. '+e.message;});});
      ...
      win.querySelector('#ai-term-suggest').addEventListener('click',function(){var out=win.querySelector('#ai-term-output');out.textContent='Thinking…';API.chat(win.querySelector('#ai-term-goal').value,[],'terminal').then(function(r){var reply=r.reply||'';var lines=reply.split('\n').map(function(line){return line.trim();}).filter(function(line){return line&&line.indexOf('```')!==0;});suggestedCommand=(lines[0]||'').replace(/^\$\s*/, '').trim();out.textContent=reply;}).catch(function(e){out.textContent='Backend Offline — run ./start-bos.sh. '+e.message;});});
      win.querySelector('#ai-term-run').addEventListener('click',function(){if(!suggestedCommand){showToast('AI Studio','Generate a command first');return;}if(!confirm('Run this command?\n\n'+suggestedCommand))return;API.exec(suggestedCommand,'~').then(function(r){win.querySelector('#ai-term-output').textContent+='\n\n'+(r.stdout||r.stderr||'Command completed.');}).catch(function(e){win.querySelector('#ai-term-output').textContent+='\n\nBackend Offline — run ./start-bos.sh. '+e.message;});});
      win.querySelector('#ai-code-send').addEventListener('click',function(){var out=win.querySelector('#ai-code-output');out.textContent='Thinking…';API.chat(win.querySelector('#ai-code-task').value,[],'coding').then(function(r){out.textContent=r.reply||r.error||'';}).catch(function(e){out.textContent='Backend Offline — run ./start-bos.sh. '+e.message;});});
      ...
      win.querySelector('#ai-voice-send').addEventListener('click',function(){var out=win.querySelector('#ai-voice-output');out.textContent='Thinking…';API.chat(win.querySelector('#ai-voice-text').value,[],'chat').then(function(r){lastVoiceReply=r.reply||'';out.textContent=lastVoiceReply;}).catch(function(e){out.textContent='Backend Offline — run ./start-bos.sh. '+e.message;});});
